...
- **Pipecat Cloud** (us-west, org `varying-tiger-jade-219`): agent **`human-virtual-agent`**, image **`haiolli/human-virtual-agent:0.9`** built at repo SHA `23166735af18` (A9 pin), profile `agent-2x`, scaling 0–10 (`min_agents=0` ⇒ ~19 s cold join measured at B9 — warm-instance decision is S74 material).
